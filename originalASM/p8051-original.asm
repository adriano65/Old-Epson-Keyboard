...
; Note: P1.0, P1.1, P1.2 also used for LEDs (active low)
...
original_version EQU 1
;little_jmp_table_mod EQU 1
...
;STACK_POINTER_POS  EQU  2Fh   ; NO 
;STACK_POINTER_POS  EQU  5Fh   ; NO 
;STACK_POINTER_POS  EQU  0CFh   ; NO 
;STACK_POINTER_POS  EQU  11h   ; NO
...
start:	
  setb	EA		; enable INTs
	mov	SP, #STACK_POINTER_POS	; Inizializza lo Stack
	; set port P1 - P1.0, P1.1, P1.2 inputs to NAND ls38 - outputs to LEDs - active low
...
waitRAM08h_tohigh:
	jnb	8h, $           ; If the bit at RAM address 08h is 0, stay here
...
label03A3:
	ajmp	label122    ; label122 is @ 03EAh
	ajmp	label92
	ajmp	label123
	ajmp	label124
	ajmp	label120
...
label118:
    ...
    mov  A, R2
    add  A, #0ECh       ; Check upper bound
    jc   label120       ; Target for R2 >= 20 (approx)
    mov  A, R2
    add  A, #0F6h       ; Check lower bound
    jnc  label121       ; Target for R2 < 10

    ; --- Direct Long Jump Dispatcher ---
    ; Replaces mov DPTR, rl A, subb A, and jmp @A+DPTR
    
    cjne R2, #0Ah, check_0Bh   ; If R2 = 10 (0Ah)
    ljmp label122
    
check_0Bh:
    cjne R2, #0Bh, check_0Ch   ; If R2 = 11 (0Bh)
    ljmp label92
    
check_0Ch:
    cjne R2, #0Ch, check_0Dh   ; If R2 = 12 (0Ch)
    ljmp label123
    
check_0Dh:
    cjne R2, #0Dh, check_0Eh   ; If R2 = 13 (0Dh)
    ljmp label124
    
check_0Eh:
    cjne R2, #0Eh, check_0Fh   ; If R2 = 14 (0Eh)
    ljmp label120              ; Note: redirected to label120 per original table
    
check_0Fh:
    cjne R2, #0Fh, check_10h   ; If R2 = 15 (0Fh)
    ljmp label125
    
check_10h:
    cjne R2, #10h, check_11h   ; If R2 = 16 (10h)
    ljmp label126
    
check_11h:
    cjne R2, #11h, check_12h   ; If R2 = 17 (11h)
    ljmp label127
    
check_12h:
    cjne R2, #12h, check_13h   ; If R2 = 18 (12h)
    ljmp label128
    
check_13h:
    cjne R2, #13h, table_end   ; If R2 = 19 (13h)
    ljmp label129
...
label111:
    ; --- Original Range Checks ---
    mov  A, R2
    add  A, #0ECh       ; If R2 < 20 (approx), carry is not set
    jc   label147
    mov  A, R2
    add  A, #0F6h       ; If R2 < 10, no carry is generated
    jnc  label148

    ; --- Direct Dispatcher (Replaces Math & DPTR Table) ---
    ; This block executes only if R2 is between 10 and 19
    
    cjne R2, #0Ah, check_0B_h   ; R2 = 10 decimal
    ljmp label149
check_0B_h:
    cjne R2, #0Bh, check_0C_h   ; R2 = 11 decimal
    ljmp label92
check_0C_h:
    cjne R2, #0Ch, check_0D_h   ; R2 = 12 decimal
    ljmp label150
check_0D_h:
    cjne R2, #0Dh, check_0E_h   ; R2 = 13 decimal
    ljmp label151
check_0E_h:
    cjne R2, #0Eh, check_0F_h   ; R2 = 14 decimal
    ljmp label147
check_0F_h:
    cjne R2, #0Fh, check_10_h   ; R2 = 15 decimal
    ljmp label152
check_10_h:
    cjne R2, #10h, check_11_h   ; R2 = 16 decimal
    ljmp label153
check_11_h:
    cjne R2, #11h, check_12_h   ; R2 = 17 decimal
    ljmp label154
check_12_h:
    cjne R2, #12h, check_13_h   ; R2 = 18 decimal
    ljmp label155
check_13_h:
    cjne R2, #13h, table_exit  ; R2 = 19 decimal
    ljmp label156
...
sendAccumulator:
	...
	clr	ET0
	clr	TR0
	jb	8h, abort_sending     ;jump if 8h is set
...
Wait_Sync_Pattern:
	jb	8h, data_received   ; If it's set (Bit 0 of RAM Byte 21h), the keyboard has incoming data from the host and go to process it
...
ifdef original_version    ; --------------------------------------------
ifndef little_jmp_table_mod

data_received:
	clr	EX0
	clr	8h        ; clear Data Ready flag
	mov	R2, 0Bh
	mov	C, 24h
	setb	EX0
	jc	parse_received
...
jmptbl9C9:
	...
	ajmp	label300
	ajmp	label301
;	ljmp	label301  ; breaks all :-(

unsupported_cmd:
	jnb	8h, $     ; wait until 8h (Data Ready) is cleared
	sjmp	data_received
...
data_received:
	...
	ljmp parse_received
skip_parse_received:
	ljmp	label278  ; CHANGED: AJMP to LJMP
...
jmptbl9C9:
	...
	ljmp	label297
	ljmp	label298
	ljmp	label286      ; Duplicate entry safe with LJMP
	ljmp	label299
	ljmp	label286      ; Duplicate entry safe with LJMP
	ljmp	label300
	ljmp	label301
...
unsupported_cmd:
	jnb	8h, $     ; wait until 8h (Data Ready) is cleared
	ljmp	data_received ; CHANGED: SJMP to LJMP

label286:
    mov 25h, #0h
    jb  22h, unsupported_cmd
    jnb 8h, skip_data_jump   ; If bit 8h is NOT set, jump over the LJMP
...
label285:
	acall	send_acknowledge
	jnz	label315
	clr	ET0
	clr	TR0
	clr	EX0
	jb	8h, enable_timer0_int0
label318:
	jb	T1, label317
